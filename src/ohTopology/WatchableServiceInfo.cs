...
namespace OpenHome.Av
{
    ...
    public abstract class ServiceInfo : Service, IServiceInfo
    {
        protected ServiceInfo(INetwork aNetwork, string aId)
        {
            iDetails = new Watchable<IInfoDetails>(aNetwork, string.Format("Details({0})", aId), new InfoDetails());
            iMetadata = new Watchable<IInfoMetadata>(aNetwork, string.Format("Metadata({0})", aId), new InfoMetadata());
            iMetatext = new Watchable<IInfoMetatext>(aNetwork, string.Format("Metatext({0})", aId), new InfoMetatext());
        }
    ...
    {
        public ServiceInfoNetwork(INetwork aNetwork, string aId, CpDevice aDevice)
            : base(aNetwork, aId)
        {
            iThread = aNetwork;
    ...
    {
        public ServiceInfoMock(INetwork aNetwork, string aId, IInfoDetails aDetails, IInfoMetadata aMetadata, IInfoMetatext aMetatext)
            : base(aNetwork, aId)
        {
            iNetwork = aNetwork;
    ...
    public class ProxyInfo : IServiceInfo, IProxy
    {
        public ProxyInfo(ServiceInfo aService, IWatchableDevice aDevice)
        {
            iService = aService;
            iDevice = aDevice;
        }
